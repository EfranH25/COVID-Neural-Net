...
if __name__ == '__main__':
    ...
    initial = pop[{'t':0}]
    initial.loc[{'group':'S'}] = rg.poisson(initMeanS.expand_dims({'k': k}, -1)) #Fill in this line with actual data of population
    initial.loc[{'group':'I'}] = multinomial(1, xr.ones_like(initial.loc[{'group':'I'}], int), 'loc').broadcast_like(initial.loc[{'group':'I'}]) #initial infected populaiton --> fill in with real data
    ...
    # if had dada we could do transport = xr.DataArray(logm([DATA])), coord = [location, locations], dims=['group', 'loc', 'outloc'] --> log used for infinitesimal rates/data


    ds = xr.Dataset({'pop': pop,
                     'transport': transport,
                      # rg.lognormal is created dummy data
                      # iRate = number of infections per daya --> Beta in SIR
                     'iRate': xr.DataArray(rg.lognormal(1, 0.1, locations.shape), dims=['loc']),
                      # rRate = chance per day someone recovers (recover/die) --> Gamma in SIR
                     'rRate': xr.DataArray(rg.lognormal(0.01, 0.001, locations.shape), dims=['loc']),
                      # dRate = for each person who recovers --> tells if whether person who recovered died or lived
